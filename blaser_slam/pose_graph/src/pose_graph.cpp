...
PoseGraph::PoseGraph()
{
  ...
  // todo optimization is disabled for now! enable this later!
...
void PoseGraph::registerPub(ros::NodeHandle &n)
{
  ...
  pub_pose_graph = n.advertise<visualization_msgs::MarkerArray>("pose_graph",
                                                                1000);
  for (int i = 1; i < 10; i++)
    pub_path[i] = n.advertise<nav_msgs::Path>("path_" + to_string(i), 1000);
  pub_undrift_odom = n.advertise<nav_msgs::Odometry>("undrift_odom", 1000);
}
...
bool PoseGraph::addKeyFrame(KeyFrame *cur_kf, bool flag_detect_loop)
{
  //! 0. In this function, loop closures are detected, verified, and then
  //!  the constraint is added to pose-graph optimization.
  //! 1. shift to base frame
  bool global_loop_closure = false;
  Vector3d vio_P_cur;
  Matrix3d vio_R_cur;
  ...
  // transform vio pose to the world frame if in a different sequence
  ...
  //Step 2. find loop closure
  int loop_index = -1;
  if (flag_detect_loop) // set to true in pose_graph_node.cpp/process()
  {
    TicToc tmp_t;
  ...
  // if found image match candidate!
  // todo delete loop_index = -1. Now I add this line to disable global loop closure.
  loop_index = -1;
  if (loop_index != -1)
  {
    ROS_DEBUG(" %d detect loop with %d \n", cur_kf->index, loop_index);
    KeyFrame *old_kf = getKeyFrame(loop_index);

    //step 2.1 if found loop closure, try to find image connection
    if (cur_kf->findConnection(old_kf))
    {
      printf("Found image connection!");
      setEarliestLoopIndex(loop_index);

      Vector3d w_P_old, w_P_cur, vio_P_cur; // position
      Matrix3d w_R_old, w_R_cur, vio_R_cur; // rotation
      ...
      // shift vio pose of whole sequence to the world frame if taken from different sequences
      ...
      //step update optimization process
      m_optimize_buf.lock();
      optimize_buf.push(cur_kf->index);
      m_optimize_buf.unlock();

      global_loop_closure = true;
    }
  }
  m_keyframelist.lock();
  Vector3d P;
  ...
  if (SHOW_L_EDGE)
  {
    if (cur_kf->has_loop)
    {
      ...
      if (cur_kf->sequence > 0)
      {
        ...
        posegraph_visualization->add_loopedge(P0, connected_P + Vector3d(
            VISUALIZATION_SHIFT_X, VISUALIZATION_SHIFT_Y, 0));
      }
...
// todo use a vector to store keyframe, so can do direct indexing.
//   Will keyframes be deleted from this list? Why use list?
KeyFrame *PoseGraph::getKeyFrame(int index)
{
//    unique_lock<mutex> lock(m_keyframelist);
  // following is used when keyframelist is a list
  /*
  ...
  */
  ...
  // following is used when keyframelist is a vector
...
int PoseGraph::detectLoop(KeyFrame *keyframe, int frame_index)
{
  ...
  // todo reject candidates that has large translation difference
  ...
  if (DEBUG_IMAGE)
  {
    for (unsigned int i = 0; i < ret.size(); i++)
    {
      ...
      putText(tmp_image, "index:  " + to_string(tmp_index) + "loop score:" +
                         to_string(ret[i].Score), cv::Point2f(10, 50),
  ...
  if (ret.size() >= 1 && ret[0].Score > 0.05)
    for (unsigned int i = 1; i < ret.size(); i++)
    {
      ...
      // to delete the translation threshold is only temporary
      // reject far apart matched pairs
      if ((getKeyFrame(ret[i].Id)->vio_T_w_i - keyframe->vio_T_w_i).norm() >
          0.1)
        continue;
    ...
    if (cur_index != -1)
    {
      ...
      for (; it != keyframelist.end(); it++)
      {
        ...
        if ((*it)->has_loop)
        {
          ...
          Vector3d euler_conncected = Utility::R2ypr(
              q_array[connected_index].toRotationMatrix());
          Vector3d relative_t;
      ...
      */
      ...
      // evaluate drift (difference between VIO pose and optimized pose)
      // this drift result is added upon new incoming pose from vio
      m_drift.lock();
      ...
      // update the landmark map based on updated
      lm_map_->updateMapPoints();
    }
...
{
  //todo updated keyframe info is in here!
  m_keyframelist.lock();
...
//        printf("path p: %f, %f, %f\n",  P.x(),  P.z(),  P.y() );
    ...
    if (SHOW_L_EDGE)
    {
      if ((*it)->has_loop && (*it)->sequence == sequence_cnt)
      {
        ...
        if ((*it)->sequence > 0)
        {
          posegraph_visualization->add_loopedge(P, connected_P + Vector3d(
              VISUALIZATION_SHIFT_X, VISUALIZATION_SHIFT_Y, 0));
        }
      }
    }
...
void PoseGraph::savePoseGraph()
{
  ...
  for (; it != keyframelist.end(); it++)
  {
    ...
    fprintf(pFile,
            ...
            (*it)->loop_index,
            (*it)->loop_info(0), (*it)->loop_info(1), (*it)->loop_info(2),
            (*it)->loop_info(3),
            (*it)->loop_info(4), (*it)->loop_info(5), (*it)->loop_info(6),
            (*it)->loop_info(7),
...
void PoseGraph::loadPoseGraph()
{
                ...
                &keypoints_num) != EOF)
  {
    ...
    */
    ...
    KeyFrame *keyframe = new KeyFrame(time_stamp, index, VIO_T, VIO_R, PG_T,
                                      PG_R, image, loop_index, loop_info,
...
void PoseGraph::addLoopClosureKFIdx(size_t kf_idx)
{
  m_optimize_buf.lock();
  optimize_buf.push(kf_idx);
  m_optimize_buf.unlock();
}

int PoseGraph::getFirstKFIdx()
{
  if (keyframelist.empty())
    return -1;
  return keyframelist[0]->index;
}

int PoseGraph::getLastKFIdx()
{
  if (keyframelist.empty())
    return -1;
  return keyframelist.back()->index;
}

void PoseGraph::setLMMap(LandmarkMap *lm_map)
{
  lm_map_ = lm_map;
}

bool PoseGraph::setEarliestLoopIndex(int kf_idx)
{
  if (earliest_loop_index > kf_idx || earliest_loop_index == -1)
  {
    earliest_loop_index = kf_idx;
    return true;
  } else
  {
    return false;
